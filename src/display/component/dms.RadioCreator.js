...
(function(dms){
    /**
     *
     * @param container 添加单选框的容器 DOM对象
     * @param data   单选框对象的数据
     * @constructor
     */
    var RadioCreator = function(container,data) {
        ...
        var radioName = "radio" + dms.getNewID();
        for(var i = 0; i <len; i++){
            ...
            //单选按钮
            var radioId = "radio" + dms.getNewID();
            var optionRadio = document.createElement("input");
            optionRadio.setAttribute("data-type", "common-check-radio");
            optionRadio.type = "radio";
            optionRadio.name = radioName;
            optionRadio.value = options[i];
            optionRadio.id = radioId;
            var optionRadioStyle = optionRadio.style;
//            optionRadioStyle.width = "40px";
//            optionRadioStyle.height = "87px";
            optionRadioStyle.display = "none";
//            optionRadioStyle.background = bdColor;
//            optionRadioStyle.marginRight = "10px";
//            optionRadioStyle.float = "left";
//            if( i == 0){      //不默认选中
//                optionRadio.setAttribute("checked", "checked");
//            }
            optionRadio.setAttribute("data-radio", "user-radio");	//添加字段输入
            optionRadio.setAttribute("data-objectId", objectId);	//添加元素 ID
            if(fbField){
                optionRadio.setAttribute("data-fb-field", fbField);
            }
